...
Pour un élément $F_i$ de la suite, la dernière addition sera donc $f_i+f_{i-1}$. Si $f_i$ est un nombre de $l$ bits. La dernière addition comportera donc $l$ opérations. La complexité de fibo2 est donc égale à la somme de toutes les opérations effectuées, un nombre $f_i$ est codé sur $l $ bits. Pour un nombre $n$,  $l=2^n.$ On peut donc exprimer la complexité de la façon suivante :
%\overline{log_2(f_i+1)}$. 
\begin{center}
	$\sum\limits_{n=1}^{n=l}2^n$ %\overline{log_2(f_i+1)}$
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	On reprend la suite $F_i$ de l'exercice précédent.
